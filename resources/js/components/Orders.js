...
import {getStateOrders} from './OrderFunctions';
...
class Orders extends Component {
    ...
    componentDidMount(){
        this.orders.orderByChild('shopId').equalTo(parseInt(this.props.shopid)).on("value", snap => {
            let data = [];
        ...
        getStateOrders().then(states => this.setState({states}))

    }

    componentDidUpdate(prevProps, prevState){
        if(prevState.states !== this.state.states && prevState.states.length > 0){
            this.playSong()
    ...
    componentWillUnmount(){
        this.orders.orderByChild('shopId').equalTo(parseInt(this.props.shopid)).off('value', snap => {
            let data = [];
            snap.forEach((row,index) => {
                data.push(row.val())
            });
    ...
    playSong = () => {
        const audio = new Audio('/coom/public/notification.mp3')
        audio.play()
    }
...
if (document.getElementById('Orders')) {
    ...
    ReactDOM.render(<Orders {...props}/>, element);
}
